...
grep -Fq -- '- packaging/openwrt/**' "$workflow" || fail "release workflow ignores OpenWrt packaging changes"
grep -Fq -- '- openwrt/client/packages/candy-client/**' "$workflow" || fail "release workflow ignores candy-client package changes"
grep -Fq -- '- openwrt/client/packages/luci-app-candy/**' "$workflow" || fail "release workflow ignores LuCI package changes"
