...
{"schema_version":2,"release_kind":"candy-runtime-openwrt-client","release_tag":"runtime-v0.4.0-r28","runtime":{"version":"0.4.0","revision":28},"source":{"repository":"reTsubasa/candy-runtime","commit":"1111111111111111111111111111111111111111"},"targets":[{"target_id":"x86_64"},{"target_id":"ipq40xx-generic-arm_cortex-a7_neon-vfpv4"}],"artifacts":[{"component":"runtime-client"},{"component":"runtime-client"},{"component":"luci"},{"component":"luci"}]}
EOF

"$root/packaging/linux/prepare_release.sh" "$tmp/linux" "$tmp/release" 0.4.0 28 reTsubasa/candy-runtime 1111111111111111111111111111111111111111 >/dev/null
(cd "$tmp/release" && sha256sum -c SHA256SUMS >/dev/null)
jq -e '.runtime_version == "0.4.0-r28" and .architecture == "aarch64" and (.runtime_url | endswith("/runtime-v0.4.0-r28/candy-server-runtime-aarch64.tar.gz"))' "$tmp/release/linux-aarch64.json" >/dev/null || fail "ARM64 manifest is invalid"
