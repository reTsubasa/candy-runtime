...
PATH="$fake_bin:$PATH" CANDY_LINUX_DIST_DIR="$dist" CANDY_SDWAN_AGENT_BINARY="$agent" \
	...
	CANDY_RELEASE_VERSION=0.4.0 CANDY_RELEASE_REVISION=55 \
...
[ "$(cat "$stage/RUNTIME-RELEASE")" = 0.4.0-r55 ] || fail "server bundle release identity is missing or invalid"
[ "$(cat "$stage/RUNTIME-ARCH")" = x86_64 ] || fail "server bundle architecture identity is missing or invalid"
[ "$(cat "$edge_stage/RUNTIME-RELEASE")" = 0.4.0-r55 ] || fail "edge bundle release identity is missing or invalid"
