...
assert.ok(source.includes('node.state!=="running"&&node.state!=="ready"&&node.state!=="ok"'), "overview must hide stale telemetry for inactive nodes");
assert.ok(source.includes("SD-WAN degraded; Candy Proxy remains active"), "overview must isolate SD-WAN degradation from a healthy Proxy fallback");
...
assert_contains "$makefile" '^PKG_RELEASE:=101$'
assert_contains "$client_makefile" '^PKG_RELEASE:=101$'
...
assert_contains "$sdwan" 'local function usable_string\(value\)'
assert_contains "$sdwan" 'value == "unavailable" or value == "unknown" or value == "not-configured"'
assert_contains "$sdwan" 'status_site_id = usable_string\(object_value\(status and status\.site, "id"\)\) or usable_string\(profile\.site_id\)'
assert_contains "$sdwan" 'status_segment_id = usable_string\(object_value\(status and status\.segment, "id"\)\) or usable_string\(profile\.segment_id\)'
