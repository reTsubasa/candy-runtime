...
FAKE_CALLS="$calls" PATH="$bin:$PATH" \
	"$product" --bootstrap-file "$bootstrap" --node 192.0.2.10 --user operator --log "$log" >"$tmp/status.json"
...
FAKE_ALREADY_REGISTERED=1 FAKE_CALLS="$calls" PATH="$bin:$PATH" \
	"$product" --bootstrap-file "$bootstrap" --node 192.0.2.10 --user operator --log "$log" >"$tmp/already.json"
...
chmod 0600 "$insecure"
if FAKE_CALLS="$calls" PATH="$bin:$PATH" "$product" --bootstrap-file "$insecure" --node 192.0.2.10 >/dev/null 2>&1; then
