...
make_bundle "$tmp/good" x86_64 0.4.0-r60
good_sha=$(sha256 "$tmp/good.tar.gz")
reset_service_state
run_upgrade --bundle-file "$tmp/good.tar.gz" --sha256 "$good_sha" --version 0.4.0-r60 >/dev/null
...
: >"$tmp/systemd.log"
if run_upgrade --bundle-file "$tmp/good.tar.gz" --sha256 "$(printf '%064d' 0)" --version 0.4.0-r60 >"$tmp/bad-sha.out" 2>&1; then fail "wrong checksum was accepted"; fi
...
make_bundle "$tmp/wrong-arch" aarch64 0.4.0-r60
: >"$tmp/systemd.log"
if run_upgrade --bundle-file "$tmp/wrong-arch.tar.gz" --sha256 "$(sha256 "$tmp/wrong-arch.tar.gz")" --version 0.4.0-r60 >"$tmp/bad-arch.out" 2>&1; then fail "wrong architecture was accepted"; fi
[ ! -s "$tmp/systemd.log" ] || fail "architecture rejection changed systemd state"
make_bundle "$tmp/wrong-version" x86_64 0.4.0-r45
: >"$tmp/systemd.log"
if run_upgrade --bundle-file "$tmp/wrong-version.tar.gz" --sha256 "$(sha256 "$tmp/wrong-version.tar.gz")" --version 0.4.0-r60 >"$tmp/bad-version.out" 2>&1; then fail "wrong release identity was accepted"; fi
...
make_bundle "$tmp/rollback-candidate" x86_64 0.4.0-r60 broken
rollback_sha=$(sha256 "$tmp/rollback-candidate.tar.gz")
reset_service_state
if FAKE_HEALTH_FAIL=1 run_upgrade --bundle-file "$tmp/rollback-candidate.tar.gz" --sha256 "$rollback_sha" --version 0.4.0-r60 >"$tmp/rollback.out" 2>&1; then fail "failed health verification was accepted"; fi
...
failed_release="$host/opt/candy/releases/0.4.0-r60-$(printf '%s' "$rollback_sha" | cut -c 1-12)"
...
make_bundle "$tmp/link-bundle" x86_64 0.4.0-r60
...
: >"$tmp/systemd.log"
if run_upgrade --bundle-file "$tmp/link-bundle.tar.gz" --sha256 "$(sha256 "$tmp/link-bundle.tar.gz")" --version 0.4.0-r60 >"$tmp/link.out" 2>&1; then fail "symlink member was accepted"; fi
