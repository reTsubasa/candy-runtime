...
fn report_runtime_telemetry(
    ...
    let telemetry = RuntimeTelemetry {
        ...
        sequence,
        lifecycle,
        dataplane_phase: status.dataplane_phase.as_deref(),
...
fn unified_proxy_users(seed: &toml_edit::DocumentMut, path: &Path) -> Result<toml_edit::Item> {
    ...
    for key in keys {
        ...
        anyhow::ensure!(seed_values.get(key) == proxy_values.get(key),
