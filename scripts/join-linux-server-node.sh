...
if [ -n "$runtime_bundle" ]; then
	...
	ssh -p "$node_port" "$ssh_target" "test \"\$(sha256sum '$remote_bundle' | awk '{print \$1}')\" = '$runtime_sha256'; stage=\$(mktemp -d); trap 'rm -rf \"\$stage\"' EXIT; tar -xzf '$remote_bundle' -C \"\$stage\"; for path in usr/local/bin/candy-server usr/local/libexec/candy-sdwan-runtime usr/local/libexec/candy-cloud-enroll; do test -x \"\$stage/\$path\"; done; sudo install -m 0755 \"\$stage/usr/local/bin/candy-server\" /usr/local/bin/candy-server; sudo install -m 0755 \"\$stage/usr/local/libexec/candy-sdwan-runtime\" /usr/local/libexec/candy-sdwan-runtime; sudo install -m 0755 \"\$stage/usr/local/libexec/candy-cloud-enroll\" /usr/local/libexec/candy-cloud-enroll; rm -f '$remote_bundle'"
	remote_bundle=
	event runtime_install succeeded
fi

ssh -p "$node_port" "$ssh_target" 'test -x /usr/local/bin/candy-server; test -x /usr/local/libexec/candy-sdwan-runtime; test -x /usr/local/libexec/candy-cloud-enroll; sudo systemctl is-active --quiet candy-server' ||
